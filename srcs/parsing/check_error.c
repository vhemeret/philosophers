...
/*   Updated: 2022/03/10 18:48:42 by vahemere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_philo_value(t_struct *data)
{
	if (data->philo > 200 || data->philo == 0)
		...
		return (0);
	else if (data->nb_eating == 0)
		return (0);
	return (1);
}

int	check_error(t_struct *data, int ac, char **av)
{
	if (ac < 5 || ac > 6)
		return (0);
